...
def image_demo(predictor, vis_folder, path, current_time, save_result):
    if os.path.isdir(path):
        files = [p for p in get_image_list(path) if p[-1] == 'g']
        logger.info("file numers :{}".format(len(files)))
    else:
        files = [path]
    files.sort()
...
def main(exp, args):
    ...
    if args.trt:
        ...
        trt_file = None
        decoder = None
    VOC_CLASSES = ("green_go", "pedestrian_crossing", "red_stop", "speed_limited", "speed_unlimited", "yellow_back")
